...
const networkFeaturesInfo = (
  <div className="space-y-3 text-slate-600">
    <p className="text-sm font-semibold text-slate-900">These are Azure network features where this service tag can be used:</p>
  ...
  return (
    ...
    >
      ...
      <div className="w-full overflow-x-auto">
        <table className="relative min-w-full table-fixed divide-y divide-slate-200" aria-label="Azure IP Ranges">
          <thead className="bg-slate-100">
            <tr className="text-left text-xs uppercase tracking-wider text-slate-500">
              <th
                className="w-[20%] px-5 py-4 font-semibold transition hover:bg-slate-200"
              ...
              <th
                className="w-[15%] px-5 py-4 font-semibold transition hover:bg-slate-200"
              ...
              <th
                className="relative w-[25%] px-5 py-4 font-semibold transition hover:bg-slate-200"
              ...
              >
                <div className="flex items-center gap-2">
                  ...
                  <Tooltip content={networkFeaturesInfo}>
                    <span className="cursor-help text-slate-400 transition hover:text-sky-600">
              ...
              >
                <td className="px-5 py-4 text-sm font-semibold text-slate-900">
                  <button
                    ...
                    className="rounded-md border border-transparent px-2 py-1 text-left text-sky-600 transition hover:border-sky-200 hover:bg-sky-100 hover:text-sky-700"
                ...
                <td className="px-5 py-4 font-mono text-sm text-slate-900">
                  ...
                  {result.ipAddress && result.ipAddress !== result.ipAddressPrefix && (
                    <span className="mt-2 inline-block rounded-md border border-sky-200 bg-sky-50 px-2 py-1 text-xs font-semibold text-sky-700">
                      {result.ipAddressPrefix.includes('/') ? 'Contains IP' : 'Matches'}: {result.ipAddress}
                    </span>
                  )}
                </td>
                <td className="px-5 py-4 text-sm text-slate-600">{result.region || '-'}</td>
                <td className="px-5 py-4 text-sm text-slate-600">{result.systemService || '-'}</td>
                <td className="px-5 py-4 text-sm text-slate-600">{result.networkFeatures || '-'}</td>
              </tr>
            ))}
          </tbody>
        </table>
